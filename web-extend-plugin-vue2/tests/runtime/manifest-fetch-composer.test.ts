...
} from '../../src/runtime/manifest-fetch-composer'
...
describe('composeManifestFetch', () => {
  ...
  it('composes left-to-right outer first', async () => {
    ...
    const a = (next: typeof inner) => async (ctx: Parameters<typeof inner>[0]) => {
      calls.push('a-in')
      const r = await next(ctx)
      calls.push('a-out')
      return r
    }
    const b = (next: typeof inner) => async (ctx: Parameters<typeof inner>[0]) => {
      calls.push('b-in')
      const r = await next(ctx)
      calls.push('b-out')
      return r
...
describe('manifestFetchCacheMiddleware', () => {
  ...
  it('caches successful responses in memory', async () => {
    ...
    const ctx = { manifestUrl: 'http://h/api/list', credentials: 'include' as RequestCredentials }
