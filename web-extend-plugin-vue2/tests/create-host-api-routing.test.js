...
import { createHostApi } from '../src/create-host-api.js'

function mockRouter(mode) {
...
describe('createHostApi routing pipeline', () => {
  ...
  it('falls back to addRoutes when addRoute missing', () => {
    const router = mockRouter('legacy')
    const api = createHostApi('pid', router)
    api.registerRoutes([{ path: '/legacy', component: {} }])
    expect(router.addRoutes).toHaveBeenCalledTimes(1)
    const added = router.addRoutes.mock.calls[0][0]
    expect(added[0].meta.pluginId).toBe('pid')
  })
